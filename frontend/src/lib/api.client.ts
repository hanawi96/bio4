...
class ApiClient {
	...
	async uploadAvatarVideo(username: string, file: File): Promise<{ videoUrl: string; storage_key: string }> {
		const formData = new FormData();
		formData.append('file', file);

		const res = await fetch(`${this.baseUrl}/upload/avatar-video/${username}`, {
			method: 'POST',
			body: formData
		});
		if (!res.ok) {
			const error = await res.json();
			throw new Error(error.error || 'Failed to upload avatar video');
		}
		return res.json();
	}

	async removeAvatarVideo(username: string): Promise<{ success: boolean }> {
		const res = await fetch(`${this.baseUrl}/upload/avatar-video/${username}`, {
			method: 'DELETE'
		});
		if (!res.ok) throw new Error('Failed to remove avatar video');
		return res.json();
	}
