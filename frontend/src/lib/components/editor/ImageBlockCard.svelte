<script lang="ts">
	...
	// Parse content to get image count and layout
	let imageCount = 0;
	let layout = 'column';
	try {
...
>
	<div class="flex items-center gap-4">
		...
		<div class="flex-1 min-w-0">
			<h3 class="font-semibold text-gray-900 text-base tracking-tight">Image Block</h3>
			<div class="flex items-center gap-2 text-sm text-gray-500 mt-0.5">
				<span class="font-medium">{imageCount} {imageCount === 1 ? 'image' : 'images'}</span>
				<span class="text-xs px-2 py-0.5 bg-gray-100 text-gray-600 rounded-full capitalize">{layout}</span>
				{#if isVisible === 0}
